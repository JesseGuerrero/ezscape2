...
do
  ...
  PACKAGE_REPOSITORY_ID=$(curl -s "${GITLAB_API_URL}/projects/${PROJECT_ID}/packages?package_name=${PROJECT_NAME}&order_by=created_at&sort=desc" | jq -r 'map(select(.name == "rs/darkan/'${PROJECT_NAME}'")) | max_by(.created_at) .id')

  # Retrieve the latest package repository release using the GitLab API
  LATEST_RELEASE=$(curl -s "${GITLAB_API_URL}/projects/${PROJECT_ID}/packages/${PACKAGE_REPOSITORY_ID}/package_files" | jq -r '. | map(select(.file_name | endswith("world-server*.jar"))) | max_by(.created_at) | .id')
