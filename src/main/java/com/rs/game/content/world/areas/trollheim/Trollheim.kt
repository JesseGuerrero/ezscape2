...
fun mapTrollheim() {
    ...
    onObjectClick(35390) { e ->
        if (e.player.isQuestComplete(Quest.DEATH_PLATEAU) && e.player.isQuestStarted(Quest.TROLL_STRONGHOLD)) {
            ...
            e.player.schedule {
                ...
                wait(2)
                if (lift && isReturning) World.sendObjectAnimation(e.getObj(), 318)
                wait(1)
                if (lift && !isReturning) World.sendObjectAnimation(e.getObj(), 318)
                wait(2)
                if (!lift) {
    ...
    onObjectClick(34395) { e ->
        when {
            e.getObj().tile.isAt(2920, 3654) -> e.player.tele(Tile.of(2907, 10035, 0))
            e.getObj().tile.isAt(2910, 3637) -> e.player.tele(Tile.of(2907, 10019, 0))
            e.getObj().tile.isAt(2857, 3578) -> e.player.tele(Tile.of(2269, 4752, 0))
            e.getObj().tile.isAt(2885, 3673) -> {
            ...
            e.getObj().tile.isAt(2847, 3688) -> e.player.tele(Tile.of(2837, 10090, 2))
            e.getObj().tile.isAt(2796, 3614) -> e.player.tele(Tile.of(2808, 10002, 0))
    ...
    onObjectClick(67752, 67679, checkDistance = false) { e ->
        if (e.objectId == 67752) {
            e.player.setRouteEvent(RouteEvent(if (e.player.x > e.getObj().x) Tile.of(3434, 4261, 1) else Tile.of(3430, 4261, 1)) {
                e.player.lock()
                e.player.resetWalkSteps()
                World.sendObjectAnimation(e.getObj(), 497)
                e.player.forceMove(if (e.player.x < e.getObj().x) Tile.of(3434, 4261, 1) else Tile.of(3430, 4261, 1), 751, 20, 75)
            })
        } else {
            val goWest = e.player.x > 3419
            e.player.setRouteEvent(RouteEvent(if (goWest) Tile.of(3423, 4260, 1) else Tile.of(3415, 4260, 1)) {
                e.player.lock()
                e.player.faceObject(e.getObj())
                e.player.schedule {
                    for (i in 0..3) {
                        e.player.anim(13495)
                        wait(3)
...
fun handleObstacleInteractions (e: ObjectClickEvent) {
    when (e.getObj().id) {
        35391 -> {
            ...
            if (e.getObj().rotation == 3 || e.getObj().rotation == 1)
                Agility.handleObstacle(e.player, 3303, 1, e.player.transform(if (e.player.x < e.getObj().x) 2 else -2, 0, 0), 1.0)
            else
                Agility.handleObstacle(e.player, 3303, 1, e.player.transform(0, if (e.player.y < e.getObj().y) 2 else -2, 0), 1.0)
        }
        3748 -> {
            if (e.getObj().rotation == 3 || e.getObj().rotation == 1)
                Agility.handleObstacle(e.player, 3377, 2, e.player.transform(if (e.player.x < e.getObj().x) 2 else -2, 0, 0), 1.0)
            else
                Agility.handleObstacle(e.player, 3377, 2, e.player.transform(0, if (e.player.y < e.getObj().y) 2 else -2, 0), 1.0)
        }
        34878 -> {
            if (e.getObj().rotation == 0 || e.getObj().rotation == 2)
                Agility.handleObstacle(e.player, if (e.player.x < e.getObj().x) 3381 else 3382, 3, e.player.transform(if (e.player.x < e.getObj().x) 4 else -4, 0, 0), 1.0)
            else
                Agility.handleObstacle(e.player, if (e.player.y < e.getObj().y) 3381 else 3382, 3, e.player.transform(0, if (e.player.y < e.getObj().y) 4 else -4, 0), 1.0)
        }
        ...
        9306, 9305 -> {
            if (e.getObj().rotation == 0 || e.getObj().rotation == 2)
                Agility.handleObstacle(e.player, if (e.player.x < e.getObj().x) 3382 else 3381, 3, e.player.transform(if (e.player.x < e.getObj().x) 4 else -4, 0, 0), 1.0)
            else
                Agility.handleObstacle(e.player, if (e.player.y < e.getObj().y) 3382 else 3381, 3, e.player.transform(0, if (e.player.y < e.getObj().y) 4 else -4, 0), 1.0)
        }
    }
}
